...
workspace-left-panel-ssh-manager-sftp-col-name = 名前
workspace-left-panel-ssh-manager-sftp-col-size = サイズ
workspace-left-panel-ssh-manager-sftp-col-modified = 更新日時
# SFTP ファイルマネージャー
sftp-browser-file-manager = ファイルマネージャー
sftp-browser-create-channel-failed = SFTPチャネルの作成に失敗しました: { $error }
sftp-browser-connection-cancelled = 接続がキャンセルされました
sftp-browser-server-config-not-found = サーバー設定が見つかりません
sftp-browser-not-connected = サーバーに接続されていません
sftp-browser-list-dir-failed = ディレクトリの一覧取得に失敗しました: { $error }
sftp-browser-delete-failed = 削除に失敗しました: { $error }
sftp-browser-filename-invalid-chars = ファイル名に無効な文字が含まれています
sftp-browser-upload-failed = アップロードに失敗しました: { $error }
sftp-browser-upload-failed-not-connected = アップロードに失敗しました: サーバーに接続されていません
sftp-browser-download-failed = ダウンロードに失敗しました: { $error }
sftp-browser-download-failed-not-connected = ダウンロードに失敗しました: サーバーに接続されていません
sftp-browser-name-empty = 名前を空にすることはできません
sftp-browser-name-invalid-separator = 無効な名前: パス区切り文字を含めることはできません
sftp-browser-rename-failed = 名前の変更に失敗しました: { $error }
sftp-browser-folder-name-empty = フォルダー名を空にすることはできません
sftp-browser-create-folder-failed = フォルダーの作成に失敗しました: { $error }
sftp-browser-target-path-invalid = 無効な対象パスです
sftp-browser-move-failed = 移動に失敗しました: { $error }
sftp-browser-drop-files-to-upload = アップロードするファイルをドロップ
sftp-menu-open = 開く
sftp-menu-download = ダウンロード
sftp-menu-rename = 名前を変更
sftp-menu-delete = 削除
sftp-menu-details = 詳細情報
sftp-dialog-cancel = キャンセル
sftp-dialog-delete-confirm-single = 「{ $name }」を削除してもよろしいですか？この操作は取り消せません。
sftp-dialog-delete-confirm-multiple = { $count } 個の項目を削除してもよろしいですか？この操作は取り消せません。
sftp-dialog-delete-title = 削除の確認
sftp-dialog-delete-button = 削除
sftp-dialog-rename-title = 名前の変更
sftp-dialog-rename-current-name = 現在の名前: { $name }
sftp-dialog-confirm-button = OK
sftp-dialog-create-folder-title = 新しいフォルダ
sftp-dialog-create-button = 作成
sftp-dialog-file-details-title = ファイルの詳細
sftp-dialog-file-type-file = ファイル
sftp-dialog-file-type-directory = ディレクトリ
sftp-dialog-file-type-symlink = シンボリックリンク
sftp-dialog-file-type-other = その他
sftp-dialog-field-type = 種類
sftp-dialog-field-size = サイズ
sftp-dialog-field-modified = 更新日時
sftp-dialog-field-permissions = 権限
sftp-dialog-field-path = パス
sftp-dialog-move-description = 「{ $name }」を { $target } に移動
sftp-dialog-move-title = ファイルの移動
sftp-dialog-move-button = 移動
sftp-dialog-overwrite-remote = リモートファイル { $name } は既に存在します。上書きしますか？
sftp-dialog-overwrite-local = 保存先ファイル { $name } は既に存在します。上書きしますか？
sftp-dialog-overwrite-title = 上書きの確認
sftp-dialog-overwrite-button = 上書き
sftp-dialog-close-transfer-panel-title = 転送パネルを閉じる
sftp-dialog-close-transfer-panel-description = 進行中の転送があります。閉じるとすべての転送が中断され、記録が消去されます。閉じてもよろしいですか？
sftp-dialog-close-button = 閉じる
sftp-transfer-state-pending = 待機中
sftp-transfer-state-in-progress = 転送中
sftp-transfer-state-completed = 完了
sftp-transfer-state-failed = 失敗
sftp-transfer-state-cancelled = キャンセル済み
sftp-transfer-panel-title = 転送 ({ $count })
sftp-ops-resolve-auth-failed = 認証の解決に失敗しました: { $error }
sftp-ops-rename-remote-temp-failed = リモート一時ファイルの名前変更に失敗しました: { $error }。一時ファイル: { $temp }
sftp-ops-rename-local-temp-failed = 名前変更に失敗しました: { $error }。ダウンロード済みの一時ファイルは次の場所に保持されます: { $temp }
sftp-ops-read-password-failed = パスワードの読み取りに失敗しました: { $error }
sftp-ops-key-auth-no-key-path = 鍵認証が選択されていますが、鍵のパスが指定されていません
sftp-backend-list-dir-failed = ディレクトリ { $path } の一覧取得に失敗しました: { $error }
sftp-backend-read-dir-entry-failed = ディレクトリエントリの読み取りに失敗しました: { $error }
sftp-backend-read-metadata-failed = メタデータの読み取りに失敗しました: { $error }
sftp-backend-delete-file-failed = ファイル { $path } の削除に失敗しました: { $error }
sftp-backend-recursive-delete-dir-failed = ディレクトリ { $path } の再帰的削除に失敗しました: { $error }
sftp-backend-create-dir-failed = ディレクトリ { $path } の作成に失敗しました: { $error }
sftp-backend-rename-failed = { $old_path } から { $new_path } への名前変更に失敗しました: { $error }
sftp-backend-resolve-path-failed = パス { $path } の解決に失敗しました: { $error }
sftp-backend-stat-file-failed = ファイル情報 { $path } の取得に失敗しました: { $error }
sftp-backend-create-dir-io-failed = ディレクトリの作成に失敗しました: { $error }
sftp-backend-upload-file-failed = ファイルのアップロードに失敗しました: { $error }
sftp-backend-open-remote-file-failed = リモートファイルのオープンに失敗しました: { $error }
sftp-backend-create-local-file-failed = ローカルファイルの作成に失敗しました: { $error }
sftp-backend-read-failed = 読み取りに失敗しました: { $error }
sftp-backend-write-failed = 書き込みに失敗しました: { $error }
sftp-backend-flush-failed = フラッシュに失敗しました: { $error }
sftp-ops-err-connection = 接続エラー: { $msg }
sftp-ops-err-operation = 操作エラー: { $msg }
sftp-ops-err-local-io = ローカルI/Oエラー: { $msg }
sftp-ops-err-no-credentials = 認証情報が見つかりません: { $msg }
sftp-ops-err-cancelled = 転送がキャンセルされました
...
workspace-left-panel-ssh-manager-forward-kind-remote = リモート
workspace-left-panel-ssh-manager-forward-kind-dynamic = 動的
workspace-left-panel-ssh-manager-forward-bind-host = バインドホスト
workspace-left-panel-ssh-manager-forward-bind-port = バインドポート
workspace-left-panel-ssh-manager-forward-target-host = ターゲットホスト
workspace-left-panel-ssh-manager-forward-target-port = ターゲットポート
workspace-left-panel-ssh-manager-forward-description = 説明(任意)
workspace-left-panel-ssh-manager-forward-add = 転送を追加
workspace-left-panel-ssh-manager-forward-empty = ポート転送はまだありません。
workspace-left-panel-ssh-manager-forward-delete = 削除
workspace-left-panel-ssh-manager-forward-invalid = 有効なバインドホストとポートを入力してください。ローカル/リモート転送ではターゲットホストとポートも必要です。
