...
impl SftpBrowserView {
    ...
    pub fn new(node_id: String, ctx: &mut ViewContext<Self>) -> Self {
        let pane_configuration =
            ctx.add_model(|_ctx| PaneConfiguration::new(crate::t!("sftp-browser-file-manager")));
    ...
    fn connect_to_server(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match server {
                    ...
                    move |me, result, ctx| {
                        ...
                        match result {
                                ...
                                match session.sftp() {
                                    ...
                                    Err(e) => {
                                        me.connection = ConnectionState::Failed(crate::t!(
                                            "sftp-browser-create-channel-failed",
                                            error = e.to_string()
                                        ));
                                        me.show_error_toast(
                                            crate::t!("sftp-browser-create-channel-failed", error = e.to_string()),
                                            ctx,
                                        );
                                    }
                                }
                            ...
                            Err(_) => {
                                ...
                                me.connection =
                                    ConnectionState::Failed(crate::t!("sftp-browser-connection-cancelled"));
                            }
                        }
                        ctx.notify();
                    },
            ...
            None => {
                self.connection =
                    ConnectionState::Failed(crate::t!("sftp-browser-server-config-not-found"));
                self.show_error_toast(crate::t!("sftp-browser-server-config-not-found"), ctx);
                ctx.notify();
            }
        }
    }
    ...
    fn refresh_dir(&mut self, ctx: &mut ViewContext<Self>) {
        let sftp = match &self.sftp {
            Some(s) => s.clone(),
            None => {
                self.show_error_toast(crate::t!("sftp-browser-not-connected"), ctx);
                ctx.notify();
                return;
            }
        };
        ...
        self.refresh_handle = self.run_blocking(
            ...
            |me, result, ctx| {
                ...
                match result {
                    ...
                    Ok(Err(e)) => {
                        me.show_error_toast(crate::t!("sftp-browser-list-dir-failed", error = e.localized()), ctx);
                    }
                    Err(_) => {}
                }
        ...
        self.run_blocking(
            ...
            move |me, result, ctx| {
                ...
                match result {
                    ...
                    Ok(Err(e)) => {
                        me.show_error_toast(crate::t!("sftp-browser-delete-failed", error = e), ctx);
                        me.refresh_dir(ctx);
                    }
                    Err(_) => {
                        // 被取消
    ...
    fn execute_upload(&mut self, local_path: &Path, ctx: &mut ViewContext<Self>) {
        ...
        let remote_path = match build_upload_remote_path(&self.current_path, &file_name) {
            Some(p) => p,
            None => {
                self.show_error_toast(crate::t!("sftp-browser-filename-invalid-chars"), ctx);
                return;
            }
        };
    ...
    fn execute_upload_confirmed(
        ...
        if let Some(sftp) = &self.sftp {
                ...
                move |me, result, ctx| {
                    ...
                    match &result {
                        ...
                        Ok(Err(e)) => {
                            ...
                            me.show_error_toast(crate::t!("sftp-browser-upload-failed", error = e.localized()), ctx);
                            ctx.notify();
                        }
                        Err(_) => {
                            ctx.notify();
            ...
            if let Some(t) = self.transfers.iter_mut().find(|t| t.id == task_id) {
                t.state = TransferState::Failed(crate::t!("sftp-browser-not-connected"));
            }
            log::error!("sftp: 上传失败: 未连接到服务器");
            self.show_error_toast(crate::t!("sftp-browser-upload-failed-not-connected"), ctx);
            ctx.notify();
        }
    }
    ...
    fn execute_download(
        ...
        if let Some(sftp) = &self.sftp {
            ...
            if let Some(handle) = self.run_blocking(
                ...
                move |me, result, ctx| {
                    ...
                    if let Ok(Err(e)) = &result {
                        ...
                        me.show_error_toast(crate::t!("sftp-browser-download-failed", error = e.localized()), ctx);
                    }
                    ctx.notify();
                },
            ) {
            ...
            self.show_error_toast(crate::t!("sftp-browser-download-failed-not-connected"), ctx);
            ctx.notify();
        }
    }
            ...
            SftpBrowserAction::ConfirmRename => {
                ...
                }) = &self.dialog
                {
                    ...
                    if new_name.is_empty() {
                        self.show_error_toast(crate::t!("sftp-browser-name-empty"), ctx);
                        return;
                    }
                    let new_path = match build_rename_path(original_path, &new_name) {
                        Some(p) => p,
                        None => {
                            self.show_error_toast(crate::t!("sftp-browser-name-invalid-separator"), ctx);
                            return;
                        }
                    };
                        ...
                        self.run_blocking(
                            ...
                            move |me, result, ctx| {
                                match result {
                                    ...
                                    Ok(Err(e)) => {
                                        me.show_error_toast(crate::t!("sftp-browser-rename-failed", error = e.localized()), ctx);
                                    }
                                    Err(_) => {}
                                }
                                ctx.notify();
            ...
            SftpBrowserAction::ConfirmNewFolder => {
                if let Some(Dialog::CreateFolder { parent_path }) = &self.dialog {
                    ...
                    if folder_name.is_empty() {
                        self.show_error_toast(crate::t!("sftp-browser-folder-name-empty"), ctx);
                        return;
                    }
                        ...
                        self.run_blocking(
                            ...
                            move |me, result, ctx| {
                                match result {
                                    ...
                                    Ok(Err(e)) => {
                                        me.show_error_toast(crate::t!("sftp-browser-create-folder-failed", error = e.localized()), ctx);
                                    }
                                    Err(_) => {}
                                }
                                ctx.notify();
            ...
            SftpBrowserAction::ConfirmMove => {
                if let Some(Dialog::Move { source, target_dir }) = &self.dialog {
                    ...
                    let target_path = match safe_join_name(target_dir, &file_name) {
                        ...
                        None => {
                            self.show_error_toast(crate::t!("sftp-browser-target-path-invalid"), ctx);
                            self.dialog = None;
                            ctx.notify();
                            return;
                        }
                        ...
                        self.run_blocking(
                            ...
                            move |me, result, ctx| {
                                match result {
                                    ...
                                    Ok(Err(e)) => {
                                        me.show_error_toast(crate::t!("sftp-browser-move-failed", error = e.localized()), ctx);
                                    }
                                    Err(_) => {}
                                }
                                ctx.notify();
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        if self.is_drag_hovering {
            let drop_hint = Text::new_inline(
                crate::t!("sftp-browser-drop-files-to-upload"),
                appearance.ui_font_family(),
