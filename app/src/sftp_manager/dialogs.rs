...
fn render_cancel_button(appearance: &Appearance, mouse_state: MouseStateHandle) -> Box<dyn Element> {
    render_button(&crate::t!("sftp-dialog-cancel"), false, appearance, SftpBrowserAction::CloseDialog, mouse_state, Some("sftp_btn:dialog_cancel"))
}
...
fn render_delete_confirm(
    ...
    let desc = if count == 1 {
        ...
        crate::t!("sftp-dialog-delete-confirm-single", name = name)
    } else {
        crate::t!("sftp-dialog-delete-confirm-multiple", count = count)
    };

    render_confirm_dialog(
        &crate::t!("sftp-dialog-delete-title"),
        &desc,
        &crate::t!("sftp-dialog-delete-button"),
...
fn render_rename(
    ...
    // 标题行
    let title_bar = render_title_bar(&crate::t!("sftp-dialog-rename-title"), appearance, close_btn_state);

    // 当前名称提示
    let hint = crate::t!("sftp-dialog-rename-current-name", name = original_name);
    ...
    let confirm_btn = render_button(
        &crate::t!("sftp-dialog-confirm-button"),
        true,
        appearance,
...
fn render_create_folder(
    ...
    // 按钮
    let confirm_btn = render_button(
        &crate::t!("sftp-dialog-create-button"),
        true,
        appearance,
...
fn render_file_details(
    ...
    // 标题行
    let title_bar = render_title_bar(&crate::t!("sftp-dialog-file-details-title"), appearance, close_btn_state);

    // 类型
    let type_str = match entry.file_type {
        crate::sftp_manager::types::FileEntryType::File => crate::t!("sftp-dialog-file-type-file"),
        crate::sftp_manager::types::FileEntryType::Directory => crate::t!("sftp-dialog-file-type-directory"),
        crate::sftp_manager::types::FileEntryType::Symlink => crate::t!("sftp-dialog-file-type-symlink"),
        crate::sftp_manager::types::FileEntryType::Other => crate::t!("sftp-dialog-file-type-other"),
    };
    ...
    rows.add_child(detail_row(&crate::t!("sftp-dialog-field-type"), &type_str, appearance));
    rows.add_child(detail_row(&crate::t!("sftp-dialog-field-size"), &format_size(entry.size), appearance));
    let modified = entry.modified.as_deref().unwrap_or("--");
    rows.add_child(detail_row(&crate::t!("sftp-dialog-field-modified"), modified, appearance));
    let permissions = entry.permissions.as_deref().unwrap_or("--");
    rows.add_child(detail_row(&crate::t!("sftp-dialog-field-permissions"), permissions, appearance));
    rows.add_child(detail_row(&crate::t!("sftp-dialog-field-path"), &entry.path.display().to_string(), appearance));
...
fn render_move_dialog(
    ...
    render_confirm_dialog(
        ...
        &desc,
        &crate::t!("sftp-dialog-move-button"),
...
fn render_overwrite_confirm(
    ...
    let desc = match direction {
        TransferDirection::Upload => crate::t!("sftp-dialog-overwrite-remote", name = target_name),
        TransferDirection::Download => crate::t!("sftp-dialog-overwrite-local", name = target_name),
    };

    render_confirm_dialog(
        &crate::t!("sftp-dialog-overwrite-title"),
        &desc,
        &crate::t!("sftp-dialog-overwrite-button"),
...
pub fn render_dialog(
    ...
    match dialog {
        ...
        Dialog::CloseTransferPanelConfirm => {
            render_confirm_dialog(
                &crate::t!("sftp-dialog-close-transfer-panel-title"),
                &crate::t!("sftp-dialog-close-transfer-panel-description"),
                &crate::t!("sftp-dialog-close-button"),
