//! SSH data sync provider — implements the SyncDataProvider trait.
//!
// author: logic
// date: 2026-05-26
...
/// The credential kinds stored in the keychain; iterated uniformly during collect/apply/orphan-cleanup.
...
/// Server data used for SSH sync (includes encrypted secrets).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl SshSyncProvider {
    /// Creates a new SshSyncProvider instance.
    pub fn new() -> Self {
        Self {
...
impl SyncDataProvider for SshSyncProvider {
        ...
        for node in &nodes {
            ...
            if node.kind == NodeKind::Server {
                ...
                if let Some(server) = server_result {
                    // Distinguish a keychain error from "user never set a password":
                    // - Ok(Some) = password exists; encrypt and upload
                    // - Ok(None) = user genuinely has no password set; write None to the field
                    // - Err     = abort the entire upload to avoid serialising a transient
                    //             keychain failure as "no password" and overwriting the real
                    //             password on other devices (PR #161 review #5)
    ...
    fn apply_data(&self, token: &str, data: &serde_json::Value) -> Result<(), SyncEngineError> {
        ...
        // ---- Phase 0 ---- Decrypt everything + collect explicit-clear list.
        // pending_secrets: remote provided ciphertext  → must write to keychain
        // explicit_clears: remote provided None        → must delete from keychain (user cleared
        //                  the password on another device; leaving the old value would cause the
        //                  local client to keep using it, violating user intent; PR #161, 7 review rounds)
        struct PendingSecret {
            node_id: String,
            kind: SecretKind,
            // Decrypted plaintext is held in `Zeroizing<String>` so it is wiped
            // from memory on drop, matching `WrittenSecret::prior_value`.
        ...
        for credential in &ssh_data.onekey_credentials {
            let kind = OneKeyCredentialKind::parse(&credential.kind).ok_or_else(|| {
                SyncEngineError::Serialization(format!(
                    "unknown onekey credential kind: {}",
                    credential.kind
                ))
            })?;
            let secret_kind = onekey_secret_kind(kind);
        ...
        // ---- Phase 0.5 ---- Topologically sort nodes so parents precede children.
        // Orphan nodes (whose parent is not in the dataset) are treated as root nodes
        // to avoid an SQLite FK violation rolling back the entire transaction.
        let sorted_nodes = topologically_sort_nodes(&ssh_data.nodes);

        // ---- Phase 0.6 ---- Collect the existing local keychain owner IDs for later orphan cleanup.
        ...
        // ---- Phase 1 ---- Write keychain first. Any failure → abort immediately; DB is untouched.
        // Track (node_id, kind, prior_value) entries so that if the DB phase fails:
        // - prior_value=Some(v) → restore the old value (avoid clobbering an existing password)
        // - prior_value=None    → delete the entry (avoid pollution)
        // True "atomic rollback" is grounded in the idempotent-overwrite semantics of secret_store.set
        // (PR #161, 3 review rounds)
        let mut written_secrets: Vec<WrittenSecret> = Vec::new();
        for s in &pending_secrets {
            // Snapshot the prior value before writing so a rollback can truly restore it.
            // A real keychain error aborts the whole flow; NoBackend (headless Linux, etc.)
            // is treated as "no prior value". This is consistent with collect_data's read_secret
            // — the same environmental constraints apply.
            let prior_value = match self.secret_store.get(&s.node_id, s.kind) {
                // store.get already returns Option<Zeroizing<String>>; use it directly to preserve zeroize semantics
                Ok(opt) => opt,
                Err(e) => {
                    // Same strictness as read_secret: any keychain error aborts to ensure rollback remains possible.
                    rollback_keychain_writes(&self.secret_store, &written_secrets);
                    return Err(SyncEngineError::Provider(format!(
                        "failed to read prior keychain value ({}, {:?}): {e}. \
                         Rolled back {} entries. Ensure the keychain is available and retry the download.",
                        s.node_id,
                        s.kind,
                        written_secrets.len()
                    )));
            ...
            if let Err(e) = self.secret_store.set(&s.node_id, s.kind, &s.value) {
                rollback_keychain_writes(&self.secret_store, &written_secrets);
                return Err(SyncEngineError::Provider(format!(
                    "failed to write to keychain ({}, {:?}): {e}. \
                     Check keychain permissions and retry the download.",
                    s.node_id, s.kind
                )));
            }
        ...
        // ---- Phase 2 ---- DB transaction: DELETE then INSERT in topological order.
        ...
        if let Err(e) = db_result {
            // DB failed → roll back the keychain writes to avoid leaving stale secrets
            // pointing to nodes that no longer exist.
            let rolled = written_secrets.len();
            rollback_keychain_writes(&self.secret_store, &written_secrets);
            return Err(SyncEngineError::Provider(format!(
                "DB write failed ({e}); rolled back {rolled} keychain entries"
            )));
        }

        // ---- Phase 3a ---- Clean up explicit-clears: the node still exists but the remote
        // set the corresponding *_encrypted field to None. The user cleared that password on
        // another device → we must delete it from the local keychain; otherwise connections
        // would continue using the old password, violating the user's intent (PR #161, 7 review rounds).
        for (node_id, kind) in &explicit_clears {
            if let Err(e) = self.secret_store.delete(node_id, *kind) {
                log::warn!(
                    "failed to clean up explicit-clear keychain entry {node_id}/{:?}: {e}",
                    kind
                );
            }
        }

        // ---- Phase 3b ---- Clean up orphan keychain entries: passwords whose owner ID existed
        // locally but has been deleted on the remote. These must be explicitly deleted; otherwise
        // a node re-created with the same UUID would read a stale password (PR #161 review #4).
        ...
        for old_id in &existing_secret_owner_ids {
            ...
            for kind in ALL_SECRET_KINDS {
                if let Err(e) = self.secret_store.delete(old_id, kind) {
                    log::warn!("failed to clean up orphan keychain entry {old_id}/{:?}: {e}", kind);
                }
            }
        }
...
/// Records a keychain entry written during apply_data Phase 1, with a prior-value snapshot
/// for true rollback. `prior_value` is held in `Zeroizing<String>` to ensure plaintext
/// passwords are wiped from memory on drop throughout the rollback path.
...
/// True rollback: for each overwritten entry:
/// - prior_value=Some → write back the old value to avoid swallowing the user's existing password
/// - prior_value=None → delete the entry to avoid an orphan
/// Any individual step failure is only logged; it does not block the caller (best-effort).
fn rollback_keychain_writes<S: SshSecretStore + ?Sized>(store: &S, written: &[WrittenSecret]) {
    for entry in written {
        ...
        if let Err(e) = res {
            log::warn!(
                "failed to roll back keychain write for {}/{:?}: {e} (secret may retain the new value or become an orphan)",
                entry.node_id,
                entry.kind
            );
        }
...
/// Reads a keychain credential.
/// - `Ok(Some)` = password exists; encrypt and upload
/// - `Ok(None)` = user never set a password (valid state); write None to the field
/// - `Err`      = keychain failure (NoBackend / Locked / permission denied)
///
/// Note: NoBackend is not treated as a fallback Ok(None). The underlying keyring crate maps
/// both a locked keychain and a completely absent backend to NoBackend, making them
/// indistinguishable (documented behaviour in keyring 3.6). Treating NoBackend as Ok(None)
/// would silently drop passwords on a transient "locked" failure → the cloud copy gets
/// cleared and is unrecoverable after a reinstall (KDF/format improvements are pending).
/// Headless Linux / CI users who genuinely have no passwords will never trigger this function
/// during upload; if Err is returned, the error message explicitly directs the user to
/// unlock or enable the keychain.
fn read_secret(
    store: &dyn SshSecretStore,
    node_id: &str,
    kind: SecretKind,
) -> Result<Option<Zeroizing<String>>, SyncEngineError> {
    // Return the `Zeroizing<String>` from the store as-is so the plaintext keeps
    // its wipe-on-drop guarantee all the way to the caller.
    match store.get(node_id, kind) {
        Ok(opt) => Ok(opt),
        Err(e) => Err(SyncEngineError::Provider(format!(
            "failed to read keychain ({node_id}, {kind:?}): {e}. \
             The keychain may be locked, or this environment has no backend \
             (headless Linux / WSL, etc.). Unlock the keychain or enable \
             secret-service / Credential Manager and retry the upload. If this \
             server genuinely does not need password sync, clear the field in the \
             SSH manager."
        ))),
    }
}
...
fn encrypt_optional(token: &str, value: Option<&str>) -> Result<Option<String>, SyncEngineError> {
    match value {
        None => Ok(None),
        // An empty string is treated as "no password" and not uploaded
        // (compatible with prior behaviour; avoids polluting the cloud with empty-string ciphertext).
...
/// BFS topological sort: parents before children. Orphan nodes whose parent_id references a
/// node outside the dataset are appended at the end with parent_id cleared, preventing an
/// SQLite FK constraint failure from rolling back the entire download.
fn topologically_sort_nodes(nodes: &[SyncNode]) -> Vec<SyncNode> {
    ...
    // Remaining nodes are either orphans (parent_id points outside the dataset) or members of a
    // cycle. Both cases are demoted to root insertion by clearing parent_id (recoverable, no data
    // loss) with an explicit log warning so the user can see that the structure was reset.
    for n in nodes {
        if !seen.contains(&n.id) {
            if has_cycle_membership(n, nodes) {
                log::warn!(
                    "apply_data: node {} is part of a reference cycle (parent_id {:?}); demoted to root node",
                    n.id,
                    n.parent_id
                );
            } else {
                log::warn!(
                    "apply_data: node {} has parent_id {:?} which does not exist in the dataset; inserting as root node",
                    n.id,
                    n.parent_id
                );
            }
...
/// Determines whether node `start` is part of a cycle (i.e. following the parent_id chain
/// eventually loops back to itself or to a previously visited node).
/// Used to distinguish "orphan" from "cycle" in log messages; capped at max_steps to prevent
/// unbounded traversal.
fn has_cycle_membership(start: &SyncNode, all: &[SyncNode]) -> bool {
    ...
    for _ in 0..max_steps {
        ...
        if !visited.insert(current.id.as_str()) {
            // Visited the same node again → cycle detected.
            return true;
        }
        match by_id.get(pid) {
    ...
    // Exceeded max_steps without terminating → there must be a cycle.
    true
}

/// Database-backed sync version store adapter.
