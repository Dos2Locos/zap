//! OS keychain abstraction — cross-platform via the `keyring` crate.
//! Windows = Credential Manager / macOS = Keychain / Linux = Secret Service.
//!
//! Account keys have the form `<node_uuid>:password` or `<node_uuid>:passphrase`.
//! They do not depend on host or username, so **renaming a node or changing its host
//! never loses the stored password**.
...
pub enum SshSecretStoreError {
    /// No keychain backend is available on this platform (common on headless Linux / WSL
    /// without Secret Service). The UI layer should prompt the user to use a private key instead.
...
/// Credential store abstraction — `KeychainSecretStore` is the default implementation;
...
pub(crate) mod test_support {
    //! In-process in-memory mock that bypasses the OS keychain — for CI and unit tests.
